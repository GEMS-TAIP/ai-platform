...
function ChatInterface() {
  ...
  const handleSendMessage = async (event) => {
    ...
    // Need to implement API call to fetch response from ChatGPT API (connect w/ aicalls.py)
    const response = axios.get('http://localhost:5000/api_call')
  ...
  return (
    <div className="chat-container">
      <header className="chat-header">Question & Answer</header>
      {
        messages.length === 0 
          && 
      ...
      <form className="chat-input" style={{ color: 'black' }} onSubmit={handleSendMessage}>
        <input
          type="text"
